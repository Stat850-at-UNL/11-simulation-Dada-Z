...
We can use the provided figure in this homework. 
![Flowchart of the game](./figure/Craps_simulation_flowchart.png)
...
-   Note: you can combine the results of rolling number of 2 and `n`.
...
-     The `Rolling number` is stored by integers. Two dices are counted into 1 rolling. 
...
simulate_craps_game <- function() {
  ...
  repeat {
    ...
    # I got an error when test this function which the `continue` showing all the way ...
    # The loops take me much time to fix it ...
...
```{r}
# Example usage
game_result <- simulate_craps_game()  # Run a single game
game_summary <- summarize_craps_game(game_result)  # Summarize the game
print(game_summary)
```
...
simulation_result <- run_craps_simulation(10)  # Simulate 10 games
